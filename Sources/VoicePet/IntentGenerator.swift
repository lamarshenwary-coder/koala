...
enum IntentGenerator {
    ...
    - Never invent an app or action outside: Mail, Messages, Finder, Safari, Notes, Calendar, none -- EXCEPT for action "open" (just launching/switching to an app, nothing else), which may target ANY real macOS application by its actual name (e.g. "Notion Calendar", "Spotify", "Discord", "Notion"), since "tell application \\"<name>\\" to activate" works for any installed app and needs no special per-app knowledge. Every OTHER action (read, compose_draft, create_event, open_url, etc.) still requires one of the six known apps -- don't guess app-specific scripting for an app you don't have worked examples for.
